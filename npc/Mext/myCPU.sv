...
module myCPU (
    ...
    wire match_mem1 = (mem1_fw_data == ex_expected_rs1_0) || (mem1_fw_data == ex_expected_rs1_1);
    wire match_mem2 = (mem2_fw_data == ex_expected_rs1_0) || (mem2_fw_data == ex_expected_rs1_1);
    wire match_wb   = (wb_data      == ex_expected_rs1_0) || (wb_data      == ex_expected_rs1_1);
    wire match_rs1  = (ex_rs1_data  == ex_expected_rs1_0) || (ex_rs1_data  == ex_expected_rs1_1);

    logic jalr_match;
    always_comb begin
        case (ex_forward_A)
            2'b11:   jalr_match = match_mem1;
            2'b10:   jalr_match = match_mem2;
            2'b01:   jalr_match = match_wb;
            default: jalr_match = match_rs1;
        endcase
    end
    ...
    // ==========================================
    // 🌟 300MHz 极致展平对齐逻辑 (Flattened Alignment)
    // 通过分离字节和半字选择器，让 Vivado 综合器能完美映射到并行的 LUT 上
    ...
    always_comb begin
        case (wb_funct3)
            3'b000: wb_rdata_ext = {{24{wb_byte_data[7]}}, wb_byte_data};  // LB 
            3'b100: wb_rdata_ext = {24'b0, wb_byte_data};                  // LBU 
            3'b001: wb_rdata_ext = {{16{wb_half_data[15]}}, wb_half_data}; // LH 
            3'b101: wb_rdata_ext = {16'b0, wb_half_data};                  // LHU 
