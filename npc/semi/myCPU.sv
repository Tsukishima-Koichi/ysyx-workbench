...
module myCPU (
    ...
    assign poison_EX1_0   = br_flush | hd_flush_RF_EX1_0 | hd_flush_RF_EX1_0x;
    ...
    // Step 2a: test HDU changes with inst1_simple_alu (no mem ops yet)
    wire can_dual = id_valid_1 && inst1_simple_alu && pc_adjacent
        && !waw_conflict && !load_use_0_to_1 && !raw_1_to_0
        && !inst0_pred_taken
        && !(inst0_mem && inst1_mem)
        && !(inst0_ctrl && inst1_ctrl);
    ...
    // HDU for inst1 (self-pipeline: inst1 RF vs inst1 pipeline)
    logic hd_stall_RF_1;
    HazardDetectionUnit hd_inst_1 (
        ...
        .stall_ID(hd_stall_RF_1), .flush_ID_EX(hd_flush_RF_EX1_1)
    );
    // Cross-check HDU: inst0 RF reads vs inst1 pipeline (for inst1 load→inst0 use)
    logic hd_stall_RF_0x, hd_flush_RF_EX1_0x;
    HazardDetectionUnit hd_inst_0x (
        .id_rs1(rf_rs1_0), .id_rs2(rf_rs2_0), .id_opcode(rf_inst_0[6:0]),
        .ex_RegWen(ex1_valid_1 & ex1_RegWen_1), .ex_WbSel(ex1_WbSel_1), .ex_rd(ex1_rd_1),
        .mem1_RegWen(br_valid_1 & br_RegWen_1), .mem1_WbSel(br_WbSel_1), .mem1_rd(br_rd_1),
        .ls_RegWen(mem1_valid_1 & mem1_RegWen_1), .ls_WbSel(mem1_WbSel_1), .ls_rd(mem1_rd_1),
        .stall_ID(hd_stall_RF_0x), .flush_ID_EX(hd_flush_RF_EX1_0x)
    );
    // Combined stall: OR all HDU outputs
    // hd_flush_RF_EX1_1 is driven by hd_inst_1 (replaces old assign to hd_flush_RF_EX1_0)
    assign hd_stall_RF = hd_stall_RF_0 | hd_stall_RF_1 | hd_stall_RF_0x;
