...
module ACTL(
    ...
    // Zba SH2ADD: funct7=0010000, funct3=100, opcode=OP.
    // Keep the base decoder intact and override it only for the exact encoding.
    wire is_sh2add = is_R & (funct7 === 7'b0010000) &
                     (funct3 === 3'b100);
    logic [3:0] base_alu_ctrl;
    ...
    assign base_alu_ctrl[3] = use_funct3 & (funct3[2] & funct3[1]);

    // Bit 2: 命中 SLTU(0100), XOR(0101), SRL(0110), SRA(0111) -> 对应 funct3 为 011, 100, 101
    assign base_alu_ctrl[2] = use_funct3 & ( (funct3 === 3'b011) | (funct3[2] & ~funct3[1]) );

    // Bit 1: 命中 SLL(0010), SLT(0011), SRL(0110), SRA(0111) -> 对应 funct3 为 001, 010, 101
    assign base_alu_ctrl[1] = use_funct3 & ( (funct3 === 3'b001) | (funct3 === 3'b010) | (funct3 === 3'b101) );

    // Bit 0: 命中 SUB(0001), SLT(0011), XOR(0101), SRA(0111), AND(1001)
    assign base_alu_ctrl[0] = use_funct3 & ( is_sub | (funct3 === 3'b010) | (funct3 === 3'b100) | is_sra | (funct3 === 3'b111) );

    assign alu_ctrl = is_sh2add ? 4'd10 : base_alu_ctrl;
