...
module IF2_ID_Reg #(parameter DATAWIDTH = 32)(
    ...
    output logic                 id_rs1_read,
    output logic                 id_rs2_read,
    output logic                 id_is_control_flow,
    output logic                 id_valid 
);
    logic if2_rs1_read, if2_rs2_read, if2_is_control_flow;

    always_comb begin
        case (if2_inst[6:0])
            `R_TYPE, `I_TYPE, `IL_TYPE, `IJ_TYPE, `S_TYPE, `B_TYPE, `CSR_TYPE:
                if2_rs1_read = 1'b1;
            default: if2_rs1_read = 1'b0;
        endcase
        case (if2_inst[6:0])
            `R_TYPE, `S_TYPE, `B_TYPE: if2_rs2_read = 1'b1;
            default: if2_rs2_read = 1'b0;
        endcase
        if2_is_control_flow = (if2_inst[6:0] == `B_TYPE) ||
                              (if2_inst[6:0] == `IJ_TYPE);
    end
    ...
    always_ff @(posedge clk) begin
        if (rst || flush) begin
            id_rs1_read       <= 1'b0;
            id_rs2_read       <= 1'b0;
            id_is_control_flow <= 1'b0;
        end else if (!stall) begin
            id_rs1_read       <= if2_rs1_read;
            id_rs2_read       <= if2_rs2_read;
            id_is_control_flow <= if2_is_control_flow;
        end
    end
endmodule
...
module ID_EX_Reg #(parameter DATAWIDTH = 32)(
    ...
    input  logic [DATAWIDTH-1:0] id_agu_precalc,
    input  logic                 id_agu_precalc_valid,
    input  logic [DATAWIDTH-1:0] id_branch_target, 
    input  logic                 id_is_M,   // 🌟 新增传入

    // 🌟 2. 新增：传给 EX 阶段
    output logic [DATAWIDTH-1:0] ex_expected_rs1_0,
    output logic [DATAWIDTH-1:0] ex_expected_rs1_1,
    
    // 🌟 1. 新增：从 ID 接收提前算好的期望值
    input  logic [DATAWIDTH-1:0] id_expected_rs1_0,
    input  logic [DATAWIDTH-1:0] id_expected_rs1_1,
    ...
    output logic [DATAWIDTH-1:0] ex_agu_precalc,
    output logic                 ex_agu_precalc_valid,
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            ...
            ex_agu_precalc_valid <= id_agu_precalc_valid;
        end
    end
    ...
    always_ff @(posedge clk) begin
        if (!stall) begin 
            ...
            {ex_mem2_load_A, ex_mem2_load_B} <= {id_mem2_load_A, id_mem2_load_B};
            ex_agu_precalc <= id_agu_precalc;
            ex_csr_idx <= id_csr_idx;
            ex_CsrImmSel <= id_CsrImmSel;
            ex_branch_target <= id_branch_target;

            // 🌟 3. 新增：顺着流水线打拍传递
            ex_expected_rs1_0 <= id_expected_rs1_0;
            ex_expected_rs1_1 <= id_expected_rs1_1;
...
module EX_MEM1_Reg #(parameter DATAWIDTH = 32)(
    ...
    input  logic                 ex_RegWen, ex_MemWen, ex_is_M,
    ...
    output logic                 mem1_RegWen, mem1_MemWen, mem1_is_M,
    ...
    always_ff @(posedge clk) begin
        if (rst) begin
            mem1_rd <= 0;
            {mem1_RegWen, mem1_MemWen, mem1_is_M} <= 0;
            mem1_WbSel <= 0;
        end else if (flush) begin
            mem1_rd <= 0;
            {mem1_RegWen, mem1_MemWen, mem1_is_M} <= 0;
            ...
            mem1_rd <= ex_rd;
            {mem1_RegWen, mem1_MemWen, mem1_is_M} <= {ex_RegWen, ex_MemWen, ex_is_M};
            mem1_WbSel <= ex_WbSel;
        end
    end
...
module MEM1_MEM2_Reg #(parameter DATAWIDTH = 32)(
    ...
    input  logic [DATAWIDTH-1:0] mem1_perip_rdata,
    ...
    output logic [DATAWIDTH-1:0] mem2_fw_data,
    output logic [DATAWIDTH-1:0] mem2_load_data,
    output logic [DATAWIDTH-1:0] mem2_load_fwd_data
);

    logic [DATAWIDTH-1:0] r_mem2_load_data;
    assign mem2_load_data     = r_mem2_load_data;
    assign mem2_load_fwd_data = r_mem2_load_data;
    ...
    always_ff @(posedge clk) begin
        if (!stall) begin
            ...
            mem2_fw_data <= mem1_fw_data;
            r_mem2_load_data <= mem1_perip_rdata;
        end
    end
endmodule
...
module MEM2_WB_Reg #(parameter DATAWIDTH = 32)(
    input  logic clk, rst, flush, stall,
    input  logic [DATAWIDTH-1:0] mem2_non_load_data,
    input  logic [DATAWIDTH-1:0] mem2_perip_rdata,
    ...
    // 🌟🌟 核心魔法：使用 DONT_TOUCH 筑起物理隔离墙 🌟🌟
    // 严禁 Vivado 将 WB 阶段复杂的对齐逻辑吸纳进 MEM2！
    logic [DATAWIDTH-1:0] r_wb_perip_rdata;
    logic [1:0]           r_wb_agu_res_1_0;
    logic [2:0]           r_wb_funct3;
    ...
    always_ff @(posedge clk) begin
        if (!stall) begin
            ...
            r_wb_perip_rdata <= mem2_perip_rdata; // 写入防 Retiming 墙
